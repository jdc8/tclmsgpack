...
proc configure {args} {
    ...
    set d [dict create msgpack $msgpack static $static ldir $ldir idir $idir config $config]
    return $d
}
proc Hinstall {} { return "?destination? ?config? ?-msgpack <path>? ?-static? ?-dynamic?\n\tInstall all packages.\n\tdestination = path of package directory, default \[info library\].\n\tconfig = Critcl target to be used\n\t-msgpack <path> = path to MessagePack\n\t-static = link MessagePack statically, default is to link dynamically\n\t-dynamic = link MessagePack dynamically" }
proc _install {args} {
    global packages
	...
	if {[string length $msgpack]} {
	    lappend rcargs -L [file join $msgpack lib] -I [file join $msgpack include]
	} else {
...
proc Hdebug {} { return "?destination? ?config? ?-msgpack <path>? ?-static? ?-dynamic?\n\tInstall debug builds of all packages.\n\tdestination = path of package directory, default \[info library\].\n\tconfig = Critcl target to be used\n\t-msgpack <path> = path to MessagePack\n\t-static = link MessagePack statically, default is to link dynamically\n\t-dynamic = link MessagePack dynamically" }
proc _debug {args} {
    global packages
    set d [eval configure $args]
    dict with d {}
...
proc _gui {} {
    global INSTALLPATH MSGPACKPATH BUILDSTATIC
    package require Tk
#    package require widget::scrolledwindow
    ...
    label  .lz -text {MessagePack Path: }
    entry  .ez -textvariable ::MSGPACKPATH
    button .bz -command [list Browse ::MSGPACKPATH] -text Browse
    checkbutton .bs -text "Link MessagePack statically" -variable BUILDSTATIC -anchor w
...
proc Install {} {
    global INSTALLPATH MSGPACKPATH NOTE
	...
	_install $INSTALLPATH -msgpack $MSGPACKPATH %s
...
proc Debug {} {
    global INSTALLPATH MSGPACKPATH
    foreach p {.ei .ez .bi .bz .bs .i .d} {
	$p configure -state disabled
    }

    set fail [catch [format {
	_debug $INSTALLPATH -msgpack $MSGPACKPATH %s
